...
const distributeShare = (list, totalStake, totalTokens, participants) => {
  ...
  for (const key of Object.keys(list)) {
    const user = list[key];
    user.neumark = totalTokens
      .mul(new web3.BigNumber(user.stake).div(totalStake))
      .floor()
      .toString(10);
    bountyUsers[key] = user;
    bountyUsers[key].name = key;
    bountyUsers[key].ethAddress = participants[key]
      ? participants[key].ethAddress
      : null;
    if (!bountyUsers[key].ethAddress)
      throw new Error(
        `There was no eth address for user ${key} in ${list[key].bounty}`
      );
    if (!web3utils.isAddress(bountyUsers[key].ethAddress))
      throw new Error(
        `Address ${bountyUsers[key].ethAddress} is not an address for user ${
          key
        } in ${list[key].bounty}`
      );
...
const formatStakeCsv = (parsedCsv, totalNeumark) => {
  ...
  bounty.Signature = {
    totalTokens: totalNeumark.mul(0.2),
    totalStake: 0,
    participants: {}
  };
  bounty.Blogpost = {
    totalTokens: totalNeumark.mul(0.3),
    totalStake: 0,
    participants: {}
  };
  bounty.Video = {
  ...
  parsedCsv.forEach(row => {
    ...
    if (Number.isNaN(stake))
      throw new Error(
        `NaN was returned during parsing:${userName} bounty program:${
          bountyCat
        }`
      );
    if (stake > 0) {
      userName = userName.replace(/ /g, "");
      if (bountyCat.toLowerCase().includes("translation")) {
        bountyCat = "Translation";
      }
      bounty[bountyCat].totalStake += stake;
      if (!bounty[bountyCat].participants[userName]) {
        bounty[bountyCat].participants[userName] = {
          stake: 0,
          bounty: bountyCat
        };
      }
...
const formatResponceCsv = responceCsv => {
  ...
  responceCsv.forEach(row => {
    let slackName = row["Your user name on the Neufund Slack"]
      .replace(/ /g, "")
      .toLowerCase();
    const ethAddress = row["ETH Address to receive your bounty NEU tokens"]
      .replace(/ /g, "")
      .toLowerCase();
...
const extractUploadInfo = participants => {
  for (const key of Object.keys(participants)) {
    if (!ethNeumarkMap[participants[key].ethAddress])
      ethNeumarkMap[participants[key].ethAddress] = {
        neumark: new web3.BigNumber(0),
        name: key
      };
    ethNeumarkMap[participants[key].ethAddress].neumark = ethNeumarkMap[
      participants[key].ethAddress
    ].neumark.plus(participants[key].neumark);
  }
  return arrayNeumark;
};
const calculateBounty = () => {
  ...
  const totalNeumark = new web3.BigNumber(totalNeumarkString);
  ...
  const parsedCsv = d3.csvParse(
    fs.readFileSync(path.resolve(stakeCsvFile), "UTF-8")
  );
  const responceCsv = d3.csvParse(
    fs.readFileSync(path.resolve(responsesCsv), "UTF-8")
  );
  ...
  for (const key in bounty) {
    ...
    calc[key] =
      participantNo > 10
        ? distributeShare(
            bounty[key].participants,
            bounty[key].totalStake,
            bounty[key].totalTokens,
            participantsInfo[key]
          )
        : distributeShare(
            bounty[key].participants,
            bounty[key].totalStake,
            bounty[key].totalTokens.mul(
              new web3.BigNumber(0.1 * participantNo)
            ),
            participantsInfo[key]
          );
  ...
  for (const key of Object.keys(ethNeumarkMap)) {
    arrayNeumark.push({
      address: key,
      neumark: ethNeumarkMap[key].neumark.floor().toString(10),
      name: ethNeumarkMap[key].name
    });
  }
  console.log(fullInfo);
  fs.writeFileSync(
    `${outputDirePath}/bounty-eth-list.csv`,
    d3.csvFormat(arrayNeumark)
  );
  fs.writeFileSync(
    `${outputDirePath}/bounty-full-info.csv`,
    d3.csvFormat(fullInfo)
  );
  console.log("done");
};
