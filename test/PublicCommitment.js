...
contract("PublicCommitment", ([lockAdmin, investor, investor2]) => {
  ...
  it("first commit sets caps", async () => {
    ...
    expect(await chain.commitment.maxAbsCap(), "max cap to max commitment").to.be.bignumber.equal(
      chain.ether(2000)
    );
    expect(await chain.commitment.minAbsCap(), "min cap to min commitment").to.be.bignumber.equal(
      chain.ether(1)
    );
  ...
  it("should be able to read Commitment parameters", async () => {
    assert.equal(await chain.commitment.startDate.call(), startTimestamp, "startDate");
    ...
    expect(await chain.commitment.minCommitment()).to.be.bignumber.equal(chain.ether(1));
