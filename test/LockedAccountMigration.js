...
contract('TestLockedAccountMigrationTarget', ([owner, investor, investor2]) => {
  ...
  it('call migrate not from source should throw', async () => {
    ...
    await migrationTarget.migrateInvestor(investor, 1, 1, startTimestamp, {from: owner}).should.be.rejectedWith(EVMThrow);
  });
  ...
  it('target with invalid source should throw', async () => {
    ...
    await chain.lockedAccount.enableMigration(migrationTarget.address).should.be.rejectedWith(EVMThrow);
  });
  ...
  async function migrateOne() {
    ...
    await migrationTarget.setMigrationSource(chain.lockedAccount.address);
    assert.equal(await migrationTarget.getMigrationFrom(), chain.lockedAccount.address, 'correct migration source set');
    let tx = await chain.lockedAccount.enableMigration(migrationTarget.address);
